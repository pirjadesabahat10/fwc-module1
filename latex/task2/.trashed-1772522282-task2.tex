...
is called the \emph{signum function}.  
...
\item[$\diamond$] \textbf{Ordered pair}  
A pair of elements grouped together in a particular order.

\item[$\diamond$] \textbf{Cartesian product}  
...
\item[$\diamond$] If $(a,b)=(x,y)$, then $a=x$ and $b=y$.

\item[$\diamond$] If $n(A)=p$ and $n(B)=q$, then $n(A\times B)=pq$.

\item[$\diamond$] $A\times\phi=\phi$.

\item[$\diamond$] In general, $A\times B\neq B\times A$.

\item[$\diamond$] \textbf{Relation}  
A relation $R$ from a set $A$ to a set $B$ is a subset of the cartesian product
$A\times B$ obtained by describing a relationship between the first element $x$
and the second element $y$ of the ordered pairs in $A\times B$.

\item[$\diamond$] The image of an element $x$ under a relation $R$ is given by $y$,
where $(x,y)\in R$.

\item[$\diamond$] The domain of $R$ is the set of all first elements of the ordered
pairs in a relation $R$.

\item[$\diamond$] The range of the relation $R$ is the set of all second elements of
the ordered pairs in a relation $R$.

\item[$\diamond$] \textbf{Function}  
...
\item[$\diamond$] $A$ is the domain and $B$ is the codomain of $f$.

\item[$\diamond$] The range of the function is the set of images.

\item[$\diamond$] A real function has the set of real numbers or one of its subsets
both as its domain and as its range.

\item[$\diamond$] \textbf{\textit{Algebra of functions}}  
...
\[
\begin{aligned}
(f+g)(x) &= f(x)+g(x), && x\in X,\\[4pt]
(f-g)(x) &= f(x)-g(x), && x\in X,\\[4pt]
(f\cdot g)(x) &= f(x)\cdot g(x), && x\in X,\\[4pt]
(kf)(x) &= k\bigl(f(x)\bigr), && x\in X,\ \text{where } k \text{ is a real number},\\[6pt]
...
\begin{center}
\textcolor{cyan}{\textbf{\textit{Historical Note}}}
\end{center}
...
time deliberately assigned a specialised use of the term \textit{function}
in the analytical sense. At the end of that month, Leibnitz replied
...
\begin{tikzpicture}
  ...
  \draw[cyan, line width=0.6pt] (-2.6,0) -- (-0.8,0);

  
  \foreach \x/\y in {0/0.15,0/-0.15,0.15/0,-0.15/0} {
    \draw[cyan, fill=cyan] (\x,\y) -- ++(0.12,0.12) -- ++(-0.12,0.12)
    -- ++(-0.12,-0.12) -- cycle;
  }

 \draw[cyan, line width=0.6pt] (0.8,0) -- (2.6,0);
\end{tikzpicture}
\end{center}
\end{document}
